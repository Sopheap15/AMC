...
data %>% 
	filter(line_total > 0) %>% view()
	select(Antibiotic, form, strength, month, line_total) %>% 
	group_by(Antibiotic, month, strength) %>% 
	mutate(line_total = sum(line_total)) %>%
	distinct(Antibiotic, month, strength, line_total) %>% 
...
da %>% 
	mutate(route=recode(route,IV="Intravenous",PO="Oral")) %>%
