...
Performing AMC monitoring provides a wide range of advantages including: evaluate the trends in antimicrobial use by identifying the baseline and follow-up monitoring of overall hospital-wide AMC, most commonly used antimicrobials, wards that use the most antimicrobials... identify the areas requiring AMS initiative and monitor the progress of AMS intervention link antimicrobial exposure to the development of AMR
...
```
Sum up of all antimicrobials, we have `r round_half_up(sum(da$ddd_1000_pyear))` DDDs/1000 patient days.

### 4.1  Individual antimicrobial consumption   
...
### 4.2.1 Monthly proportion of AWaRe antibiotic consumption
...
data %>% 
	...
	labs(x = "Month", y = "Proportion (%)", title = "Monthly proportion of AWaRe antibiotic consumption") +
	theme_bw() +
...
### 4.2.2 Quartely proportion of AWaRe antibiotic consumption
```{r quartely}
data %>%
	filter(!is.na(monitoring)) %>% 
	...
	labs(x = "Quarter", y = "Proportion (%)", title = "Quartely proportion of AWaRe antibiotic consumption") +
	theme_bw() +
	theme(plot.title = element_text(color = "black", face = "bold", hjust = 0.5),
				axis.text = element_text(color = "black", face = "bold"),
				legend.title = element_blank()) +
...
### 4.2.3 Yearly consumption of AWaRe antibiotics
```{r AB AWaRe}
...
da %>% 
	...
	labs(title = "Individual consumption of Access antibiotics", 
...
da %>% 
	...
	geom_bar(stat = "identity") + facet_wrap(~route) +
	geom_text(aes(label = format(round_half_up(n), big.mark = ",")), hjust = -0.2, size = 3) +
	labs(title = "Individual consumption of Watch antibiotics", 
			 x = "Antimicrobial", y = "DDDs/1000 patient days") +
	scale_y_continuous(expand = expansion(mult = c(0.01, .15))) +
	coord_flip() +
	theme_classic() +
